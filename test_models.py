    ...
    def initialize_models(self):
        ...
        model_configs = {
            ModelType.MBERT.value: ModelConfig(
                "mBERT",
                os.path.join(self.current_dir, "fin_sbest__MBERT__lr5e-05_ep149_bs16"),
                os.path.join(self.current_dir, "tkn_for_fin_sbest__MBERT__lr5e-05_ep149_bs16")
            ),
            ModelType.XLMR.value: ModelConfig(
                "XLM-R",
                os.path.join(self.current_dir, "fin_best__XLM-RoBERTa__lr3e-05_ep65_bs32"),
                os.path.join(self.current_dir, "tkn_for_fin_best__XLM-RoBERTa__lr3e-05_ep65_bs32")
            ),
            ModelType.BERT.value: ModelConfig(
                "BERT",
                os.path.join(self.current_dir, "fin_tbest__BERTB__lr5e-05_ep94_bs16"),
                os.path.join(self.current_dir, "tkn_for_fin_tbest__BERTB__lr5e-05_ep94_bs16")
            )
        }
...
def measure_model_performance(text_samples: list[str], iterations: int = 5) -> dict:
    """Measures average inference time and response time reduction per message"""
    metrics = {}
    
    # Single warm-up pass for all models
    for classifier in model_server.models.values():
        classifier.predict(text_samples[0])
    
    # Measure inference times
    for model_name, classifier in model_server.models.items():
        times = []
        for _ in range(iterations):
            start = time.perf_counter()
            for text in text_samples:
                classifier.predict(text)
            times.append((time.perf_counter() - start) * 1000 / len(text_samples))
    ...
    # Calculate response time reduction compared to slowest model
    slowest = max(m["avg_inference_ms"] for m in metrics.values())
    for m in metrics.values():
        reduction = ((slowest - m["avg_inference_ms"]) / slowest) * 100
        m["response_time_reduction"] = f"{round(reduction, 2)}%"
...
def get_performance():
    samples = ["Hello world", "URGENT: Prize winner!"]  # Minimal test set
    return jsonify(measure_model_performance(samples))
